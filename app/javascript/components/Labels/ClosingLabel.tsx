...
const ClosingLabel = ({ children, color, className }: ClosingLabelProps) => {
	...
	function closeLabel(e) {
		...
		const parentClassName: HTMLElement = e.target.parentNode
	...
	return (
		<div
			className={ classnames('max-w-max flex items-center justify-between py-1 pl-3 pr-1 rounded-full last:mr-0 mr-1 transition-all duration-300', divBgColors, className) }
		>
			<span
				className={ classnames('text-xs font-semibold uppercase mr-2', textColors) }
			>
				{ children }
			</span>
			<span
