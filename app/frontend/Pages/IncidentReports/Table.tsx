...
const IncidentReportTable = (props: TableProps) => {
	return (
		<Table>
			<Table.Head>
				<Table.Row>
					<Table.HeadCell sort="occured_at">Occured_at</Table.HeadCell>
			...
			<Table.Body>
				<Table.RowIterator render={ (incident_report: Schema.IncidentReportsIndex) => (
					<Table.Row key={ incident_report.id }>
						<Table.Cell>
							<Link href={ Routes.incidentReport(incident_report.id) }>{ incident_report.occured_at }</Link>
						</Table.Cell>
						
						
						<Table.Cell>
							<Link href={ Routes.incidentReport(incident_report.id) }>{ incident_report.reported_at }</Link>
						</Table.Cell>
						<Table.Cell>
							<Link href={ Routes.incidentReport(incident_report.id) }>{ incident_report.agency_notified_at }</Link>
						</Table.Cell>
						
						<Table.Cell>
							<Link href={ Routes.incidentReport(incident_report.id) }>{ incident_report.location }</Link>
						</Table.Cell>
